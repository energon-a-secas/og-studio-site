...
//
//   node scripts/record-gif.mjs <target> [flags]
//
...
//                      {"type":{"sel":"...","text":"..."}}]
//
// A preview that exists in PREVIEW_MAP but not on disk (the map has 14 of
// those today) deploys without --force: filling a hole is not overwriting.
...
function parseArgs(argv) {
  const a = { target: null, out: null, deploy: false, card: null, force: false,
    size: '646x300', fps: 10, scenario: null };
  ...
  if (!a.target) throw new Error('usage: record-gif.mjs <card|og-id|domain|url> [flags]');
  return a;
}
...
function resolveTarget(target, cardFlag) {
  ...
  const byDomain = new Map([...cards].map(([id, dom]) => [dom, id]));
  if (target.includes('://')) {
  ...
  if (cards.has(target)) {
    return { url: `https://${cards.get(target)}/`, card: target, name: target };
  }
  const og = ogSites();
  if (og.has(target)) {
...
async function record({ url, outW, outH, fps, steps }) {
...
function encode(frames, outW, outH, fps) {
...
async function main() {
  ...
  const { url, card, name } = resolveTarget(a.target, a.card);
  const steps = a.scenario
    ? JSON.parse(fs.readFileSync(a.scenario, 'utf8'))
    : DEFAULT_SCENARIO;

  console.log(`${name}: demo gif of ${url}`);
  const frames = await record({ url, outW, outH, fps: a.fps, steps });
  const gifBuf = encode(frames, outW, outH, a.fps);
  ...
  if (kb > 1600) console.log('  ⚠ heavier than the heaviest existing preview — consider --fps 8 or fewer frames');
